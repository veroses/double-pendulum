...
class DoublePendulum(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = gym.spaces.Box(low=[0.0, 0.0, -40*np.pi, -40*np.pi], high=[2*np.pi, 2*np.pi, 40*np.pi, 40*np.pi], shape=(4,), dtype=np.float64) #angl and omega values
